...
public class TempUserRepository {
    ...
    public List<TempUser>findAllByOrganizationId(Long organizationId){
        ...
        return (List<TempUser>)namedParameterJdbcTemplate.query(query, params, new InvitedUserSetExtractor());
    }

    public TempUser findOne(Long id){
        String query = "SELECT * FROM temp_user WHERE id=:id";
        MapSqlParameterSource params = new MapSqlParameterSource();
        params.addValue("id", id);
        List<TempUser> tempUsers = (List<TempUser>)namedParameterJdbcTemplate.query(query, params, new InvitedUserSetExtractor());
        if(tempUsers.size() == 1){
    ...
    public TempUser findOneByActivationKey(String activationKey){
        String query = "SELECT * FROM temp_user tu INNER JOIN organization_user ou ON tu.id = ou.temp_user_id AND ou.activated = false WHERE tu.activation_key=:activationKey";
        MapSqlParameterSource params = new MapSqlParameterSource();
    ...
    public List<TempUser>findAllByOrganizationIdAndEmail(Long organizationId, String email){
        String query = "SELECT * FROM temp_user iu INNER JOIN organization_user ou ON iu.id = ou.temp_user_id AND ou.activated = false WHERE ou.organization_id = :organizationId AND email=:email";
        MapSqlParameterSource params = new MapSqlParameterSource();
        params.addValue("organizationId", organizationId);
    ...
    public void update(TempUser tempUser){
        String query = "UPDATE temp_user SET first_name=:firstName, last_name = :lastName, email=:email, last_modified_by=:modifiedBy, last_modified_date=NOW() WHERE id=:id";
        MapSqlParameterSource params = new MapSqlParameterSource();
        params.addValue("firstName", tempUser.getFirstName());
        params.addValue("lastName", tempUser.getLastName());
        params.addValue("email", tempUser.getEmail());
