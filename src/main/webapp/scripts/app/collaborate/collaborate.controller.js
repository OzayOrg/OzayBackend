...
angular.module('ozayApp')
    .controller('CollaborateController', function($scope, $state, Collaborate, MenuSearchState, Page, UserInformation) {
        $scope.button = true;
        $scope.role = [];
        $scope.memberList = [];
        ...
        $scope.contentTitle = 'Collaboration Create';

        Page.get({
            state: $state.current.name
        }).$promise.then(function(data) {
            $scope.roleList = data.roles;
            $scope.subjects = data.collaborate;
        ...
        $scope.submit = function() {
            ...
            if (confirm("What Would you like to send to " + $scope.selectedUsers.length + " people?")) {
                ...
                $scope.track = function(collaborate){
                            Collaborate.update(collaborate, function(data) {
                                              collaborate.track = !collaborate.track;
                                                $scope.track = true;
                                            }, function(error) {
                                                $scope.errorTextAlert = "Error! Please try later.";
                                            }).$promise.finally(function() {
